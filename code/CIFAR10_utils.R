...
acquire_observations <- function(
  ...
  for(i in 1:n_acq_steps){
    ...
    CIFAR10_samples_file_name = paste0(dest_folder, "CIFAR10_samples_", acq_fun, "_", i_str, ".npy")
    ...
    if(is.null(train_pool_ix[[i]])){
      ...
      if(!file.exists(CIFAR10_samples_file_name)){
        ...
        if(file.exists(model_file_name)){
          ...
          saveRDS(history, paste0(dest_folder, "CIFAR10_train_history_", acq_fun, "_", i_str, '.rds'))
        ...
        cat("\t\t\tSaving pool samples in", CIFAR10_samples_file_name, "\n")
        np$save(CIFAR10_samples_file_name, MC_samples)
        cat("\t\t\tPool samples saved in", CIFAR10_samples_file_name, "\n")
      } else {
        # If there is an MC samples file, then just load it
        cat("\t\t\tLoading pool samples from ", CIFAR10_samples_file_name, "\n")
        MC_samples = np$load(CIFAR10_samples_file_name)
        cat("\t\t\tSamples loaded ", CIFAR10_samples_file_name, "\n")
      }
      ...
      # if(file.exists(CIFAR10_samples_test_file_name)){
      #   # test set MC samples file exists
      #   cat("\t\t\tLoading test samples from", CIFAR10_samples_test_file_name, "\n")
      #   MC_samples_test = np$load(CIFAR10_samples_test_file_name)
      # } else{
      #   cat("\t\t\tComputing test samples\n")
      #   MC_samples_test = get_mc_predictions(model, x_test, nb_iter=nb_MC_samples, batch_size=256)
      #   cat("\t\t\tSaving test samples in", CIFAR10_samples_test_file_name, "\n")
      #   np$save(CIFAR10_samples_test_file_name, MC_samples_test)
      #   cat("\t\t\tTest samples saved in", CIFAR10_samples_test_file_name, "\n")
      # }
      # 
